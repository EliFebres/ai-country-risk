...
| The same matrix, **the other four rows** (`gpt-5.4-mini` 7, `gpt-4.1-mini` 8, `gpt-5.6-luna` 11, `gpt-4.1-nano` 20) | **Prose only, and now doubtful.** They were measured on the same one-payload instrument that reported `gpt-4o` as exact, and that reading did not survive contact with two more payloads. Re-runnable for about $0.20 each; until then treat them as lower bounds. |
